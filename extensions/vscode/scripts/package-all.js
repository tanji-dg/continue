...
const specificPlatform = specificPlatformArg
  ? specificPlatformArg.split("=")[1]
  : null;
...
if (specificPlatform) {
  if (specificPlatform === "all") {
    // "all"が指定された場合
    targetPlatforms = [...ALL_PLATFORMS];
    platformsToBuild = [...ALL_PLATFORMS];
    console.log(
      `[info] すべてのプラットフォームをビルドします: ${targetPlatforms.join(", ")}`,
    );
  } else {
    targetPlatforms = [specificPlatform];
    platformsToBuild = [specificPlatform];
    console.log(
      `[info] 指定されたプラットフォーム: ${specificPlatform} のみをビルドします`,
    );
  }
} else {
  targetPlatforms = [...ALL_PLATFORMS];
  ...
  if (args.includes("--current-platform-only")) {
    ...
    console.log(
      `[info] --current-platform-onlyオプション: 現在の環境 ${currentPlatform} のみをビルドします`,
    );
  }
}
...
function checkBinarySize(platform) {
  ...
  console.log(
    `[info] ${platform}: 合計サイズ: ${(totalSize / 1024 / 1024).toFixed(2)} MB`,
  );
}
  ...
  pathsToClean.forEach((pathToClean) => {
    if (fs.existsSync(pathToClean)) {
      try {
        ...
        console.warn(
          `[warn] クリーンアップエラー ${pathToClean}: ${e.message}`,
        );
      }
    }
  });
...
(async () => {
  for (const platform of platformsToBuild) {
    ...
    console.log(
      `[info] プラットフォーム ${platform} 用の強制クリーンアップを実行中...`,
    );
    forceCleanup();
    ...
    if (fs.existsSync(buildPath)) {
      try {
        ...
        console.warn(
          `[warn] build ディレクトリ内クリーンアップエラー: ${e.message}`,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[info] プラットフォーム ${platform} 用のprepackage実行中...`,
      );
      execSync(
        `node scripts/prepackage.js --target ${platform} --cross-platform`,
        {
          stdio: "inherit",
        },
      );
      ...
      console.log(
        `[success] ✅ プラットフォーム ${platform} 用のパッケージ作成が完了しました`,
      );
    } catch (error) {
      console.error(
        `[error] ❌ プラットフォーム ${platform} 用のパッケージ作成中にエラーが発生しました:`,
      );
  ...
  if (fs.existsSync(buildDir)) {
    ...
    for (const file of files) {
      ...
      const stats = fs.statSync(filepath);
      console.log(
        `[info] ${file}: ${(stats.size / 1024 / 1024).toFixed(2)} MB`,
      );
    }
  }
})();
