...
function cleanupUnusedPlatformBinaries(targetPlatform) {
  console.log(
    `[info] Cleaning up unused platform binaries for ${targetPlatform}`,
  );
  ...
  try {
      ...
      for (const platform of allPlatforms) {
        if (platform !== targetPlatform && platform !== ".gitkeep") {
          ...
          if (fs.statSync(platformPath).isDirectory()) {
            ...
            console.log(
              `[info] Removed unused @esbuild/${platform} (${(sizeBeforeCleanup / 1024 / 1024).toFixed(2)} MB)`,
            );
          }
        }
      }
    ...
    if (fs.existsSync(lancedbDir)) {
      const expectedDir = `${targetPlatform}${isWinTarget ? "-msvc" : isLinuxTarget ? "-gnu" : ""}`;
      ...
      for (const dir of allDirs) {
        if (dir !== expectedDir && dir !== ".gitkeep") {
          ...
          if (fs.statSync(dirPath).isDirectory()) {
            ...
            console.log(
              `[info] Removed unused @lancedb/${dir} (${(sizeBeforeCleanup / 1024 / 1024).toFixed(2)} MB)`,
            );
          }
        }
      }
    ...
    if (isWinTarget) {
      ...
      if (fs.existsSync(rgPath)) {
        fs.rmSync(rgPath);
        console.log(
          `[info] Removed unused rg binary (keeping rg.exe for Windows)`,
        );
      }
    }
  } catch (error) {
...
function getFolderSize(folderPath) {
  ...
  try {
    ...
    console.warn(
      `[warn] フォルダサイズ計算エラー (${folderPath}): ${e.message}`,
    );
  }
...
function performAdditionalOptimizations() {
  ...
  try {
    ...
    console.warn(
      `[warn] Error during additional optimizations: ${error.message}`,
    );
  }
}
...
function removeUnnecessaryNodeModuleFiles() {
  ...
  for (const pattern of patternsToRemove) {
    try {
      ...
      if (totalSize > 0) {
        console.log(
          `[info] Removed ${files.length} files matching ${pattern} (${(totalSize / 1024 / 1024).toFixed(2)} MB)`,
        );
      }
    } catch (error) {
      // globが無い場合はスキップ
      console.warn(
        `[warn] Could not remove pattern ${pattern}: ${error.message}`,
      );
    }
  }
}
...
void (async () => {
  ...
  const esbuildBinPath = `out/node_modules/esbuild/bin`;
  if (fs.existsSync(esbuildBinPath)) {
    fs.rmSync(esbuildBinPath, { recursive: true });
  }
  ...
  try {
    ...
    if (fs.existsSync(tmpDir)) {
      ...
      console.log(
        `[info] Final cleanup: removed tmp directory (${(tmpSize / 1024 / 1024).toFixed(2)} MB)`,
      );
    }
  } catch (error) {
